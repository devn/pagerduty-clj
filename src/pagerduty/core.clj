...
(defn safe-parse
  "Takes a response and checks for certain status codes. If 204, return nil.
   If 400, 401, 204, 422, 403, 404 or 500, return the original
   response with the body parsed as json. Otherwise, parse and return
   the body if json, or return the body if raw."
...
(defn make-request [method end-point positional
  ...
  (let [req (merge-with merge
        ...
        proper-query (dissoc query "auth" "token" "all_pages" "accept")
